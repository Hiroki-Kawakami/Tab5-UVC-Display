...
class AVIPlayer {
    let decoder: IDF.JPEG.Decoder<UInt16>
    ...
    init(
        ...
        frameBuffer: UnsafeMutableBufferPointer<UInt16>,
        audio: M5StackTab5.Audio
    ) throws(IDF.Error) {
        self.decoder = try IDF.JPEG.createDecoderRgb565(rgbElementOrder: .bgr, rgbConversion: .bt709)
        ...
        config.audio_set_clock_cb = { (rate, bits, ch, arg) in
            Log.info("rate: \(rate), bits: \(bits), ch: \(ch)")
            try! Unmanaged<AVIPlayer>.fromOpaque(arg!).takeUnretainedValue().audio.reconfigOutput(rate: rate, bps: UInt8(bits), ch: UInt8(ch))
        }
    ...
    var lastTime: Int64? = nil
    var frameCount = 0
    func videoCallback(data: UnsafeMutablePointer<frame_data_t>) {
        ...
        do throws(IDF.Error) {
            let _ = try decoder.decode(inputBuffer: inputBuffer, outputBuffer: frameBuffer)
            display.drawBitmap(start: (0, 0), end: (720, 1280), data: frameBuffer.baseAddress!)
        } catch {
...
func main() {
    ...
    do throws(IDF.Error) {
        ...
        Task.delay(1000)
        try mscDriver.mount(path: "/sdcard", maxFiles: 25)

        let player = try AVIPlayer(
            display: tab5.display,
